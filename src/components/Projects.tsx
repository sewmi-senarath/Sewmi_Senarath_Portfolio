...
const Projects = () => {
  ...
  return (
    <section id="projects" className="py-12 sm:py-16 bg-gradient-to-b from-orange-50 to-white transition-all duration-700 ease-in-out">
      <div className="container mx-auto px-3 sm:px-6 lg:px-8">
        <div className="text-center mb-8 sm:mb-16">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4 text-black transition-all duration-500">
            Featured Projects
          </h2>
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-4 sm:mt-8 px-4">
            {categories.map((category) => (
              <Button
                key={category}
                variant={selectedCategory === category ? "default" : "outline"}
                onClick={() => setSelectedCategory(category)}
                size="sm"
                className={`transition-all duration-500 transform hover:scale-110 text-xs sm:text-sm px-3 py-2 flex-shrink-0 ${
                  selectedCategory === category
                    ? "bg-black text-white shadow-lg shadow-orange-500/25 hover:shadow-orange-500/40 hover:bg-orange-500"
                    : "border-gray-400 text-gray-700 hover:border-orange-400 hover:text-orange-600 hover:bg-orange-50"
                }`}
        ...
        {/* Projects Grid - Mobile-first layout */}
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
          {filteredProjects.map((project, index) => (
            <Card
              key={project.id}
              className={`group relative overflow-hidden bg-gradient-to-br ${project.cardGradient} border-orange-200 hover:border-orange-300 transition-all duration-700 hover:shadow-2xl hover:shadow-orange-500/20 transform hover:scale-105 backdrop-blur-sm ${
                hoveredProject === project.id ? 'scale-105' : ''
              }`}
              onMouseEnter={() => setHoveredProject(project.id)}
              onMouseLeave={() => setHoveredProject(null)}
            ...
            >
              ...
              <div className="relative overflow-hidden">
                {/* Image display with fallback - Mobile responsive */}
                <div className="h-32 sm:h-40 lg:h-48 relative">
                  ...
                  >
                    <div className="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition-all duration-700" />
                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
                    <div className="text-white/80 text-2xl sm:text-4xl lg:text-6xl font-bold group-hover:scale-110 transition-all duration-500 relative z-10">
                      {project.title.split(' ').map(word => word[0]).join('').slice(0, 2)}
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute top-2 right-2 transform group-hover:scale-110 transition-all duration-500">
                  {project.isLeadership ? (
                    <Badge className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white border-none hover:from-yellow-400 hover:to-orange-400 transition-all duration-300 text-xs">
                      <Award className="w-2 h-2 sm:w-3 sm:h-3 mr-1" />
                      <span className="hidden sm:inline">Leadership</span>
                      <span className="sm:hidden">Lead</span>
                    </Badge>
                  ) : (
                    <Badge className="bg-green-500/90 text-white border-none hover:bg-green-400 transition-all duration-300 text-xs">
                ...
                <div className="absolute top-2 left-2 transform group-hover:scale-110 transition-all duration-500">
                  <Badge variant="outline" className="border-white/30 text-white bg-black/30 backdrop-blur-sm hover:bg-black/50 hover:border-white/50 transition-all duration-300 text-xs">
                    {project.category}
                  </Badge>
                </div>
              </div>
              ...
              <CardContent className="p-3 sm:p-4 lg:p-6 relative z-10">
                <h3 className="text-sm sm:text-lg lg:text-xl font-bold text-black mb-2 group-hover:text-gray-800 transition-colors duration-500 transform group-hover:scale-105">
                  {project.title}
                  {project.isLeadership && (
                    <Users className="inline w-3 h-3 sm:w-4 sm:h-4 ml-2 text-orange-500 group-hover:text-orange-600 group-hover:scale-125 transition-all duration-300" />
                  )}
                </h3>

                <p className="text-gray-600 text-xs sm:text-sm mb-3 sm:mb-4 line-clamp-2 group-hover:text-gray-700 transition-colors duration-300">
                  {project.description}
                </p>

                {/* Technologies - Mobile responsive */}
                <div className="flex flex-wrap gap-1 mb-3 sm:mb-4">
                  {(expandedTechnologies.has(project.id) ? project.technologies : project.technologies.slice(0, 2)).map((tech) => (
                    <Badge
                      key={tech}
                      variant="secondary"
                      className="text-xs bg-orange-500/20 text-orange-600 border-orange-500/30 hover:scale-110 hover:bg-orange-400/30 hover:text-orange-700 hover:border-orange-400/50 transition-all duration-300"
                  ...
                  {project.technologies.length > 2 && (
                    <button
                      onClick={() => toggleTechnologies(project.id)}
                      className="text-xs bg-gray-500/20 text-gray-600 border border-gray-500/30 hover:scale-110 transition-all duration-300 px-1.5 py-0.5 rounded-full hover:bg-orange-500/20 hover:text-orange-600 hover:border-orange-500/30"
                    >
                      {expandedTechnologies.has(project.id) 
                        ? 'Less' 
                        : `+${project.technologies.length - 2}`
                ...
                <div className="mb-4 sm:mb-6">
                  <h4 className="text-xs sm:text-sm font-semibold text-gray-700 mb-2 group-hover:text-gray-800 transition-colors duration-300">Features:</h4>
                  <ul className="text-xs text-gray-600 space-y-1 group-hover:text-gray-700 transition-colors duration-300">
                    {project.features.slice(0, 3).map((feature, idx) => (
                      <li key={idx} className="flex items-center group/feature">
                        <Star className="w-2 h-2 sm:w-3 sm:h-3 mr-1 sm:mr-2 text-orange-500 fill-current group-hover/feature:text-orange-600 group-hover/feature:scale-125 transition-all duration-300 flex-shrink-0" />
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    onClick={() => handleLinkClick(project.github)}
                    disabled={project.github === "#"}
                    className="flex-1 border-gray-400 text-gray-700 hover:border-orange-400 hover:text-orange-600 hover:bg-orange-50 transition-all duration-500 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed text-xs sm:text-sm py-1.5 sm:py-2"
                  >
                  ...
                  <Button
                    ...
                    onClick={() => handleLinkClick(project.demo)}
                    disabled={project.demo === "#"}
                    className="flex-1 bg-black text-white hover:bg-orange-500 border-none transition-all duration-500 transform hover:scale-105 hover:shadow-lg hover:shadow-orange-500/30 disabled:opacity-50 disabled:cursor-not-allowed text-xs sm:text-sm py-1.5 sm:py-2"
                  >
        ...
        <div className="text-center mt-8 sm:mt-16 px-4">
          <div className="inline-flex items-center justify-center p-4 sm:p-8 bg-gradient-to-br from-orange-50 to-white rounded-2xl border border-orange-200 backdrop-blur-sm transition-all duration-700 hover:scale-105 hover:shadow-2xl hover:shadow-orange-500/20 group overflow-hidden relative w-full max-w-2xl">
            <div className="absolute inset-0 bg-gradient-to-r from-orange-100/0 via-orange-50/50 to-orange-100/0 opacity-0 group-hover:opacity-100 transition-all duration-700"></div>
            
            <div className="relative z-10 text-center">
              <h3 className="text-lg sm:text-2xl font-bold text-black mb-2 group-hover:text-gray-800 transition-colors duration-500">
                Looking for Software Engineering Internship Opportunities
              </h3>
              <p className="text-sm sm:text-base text-gray-600 mb-2 sm:mb-4 group-hover:text-gray-700 transition-colors duration-500">
                Ready to contribute to innovative projects and grow with your team
              </p>
            </div>
          </div>
